...
export function Topbar({ title, onMenu, onReset, onNavigate, profile, settings }: { title: string; onMenu: () => void; onReset: () => void; onNavigate: (id: string) => void; profile: ClinicProfile; settings: Settings }) {
  ...
  return <header className="topbar sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200 bg-white/90 px-4 backdrop-blur"><div className="flex items-center gap-3"><Button variant="ghost" className="lg:hidden" onClick={onMenu}><Menu size={18} /></Button><img src="/logo.svg" alt="KUMPC Finance logo" className="h-9 w-9 rounded-xl shadow-sm lg:hidden" /><div><p className="text-sm text-slate-500">{profile.name}</p><h2 className="font-semibold text-slate-900">{title}</h2></div></div><div className="hidden items-center gap-2 md:flex"><Badge variant="outline">Periode aktif: {periodText}</Badge><Badge variant="green"><ShieldCheck size={13} /> Tersimpan lokal</Badge><div className="relative"><Search className="absolute left-3 top-2.5 text-slate-400" size={16} /><Input value={query} onChange={e => { setQuery(e.target.value); setOpen(e.target.value.length >= 2); }} onFocus={() => query.length >= 2 && setOpen(true)} placeholder="Cari global..." className="w-52 pl-9" /></div><Button variant="outline" onClick={quickExport}><Download size={16} />Quick Export</Button><Button variant="outline" onClick={() => { resetDemoData(); onReset(); }}><RotateCcw size={16} />Reset Demo</Button></div><Dialog open={open}><div className="mb-4 flex items-center justify-between"><div><b>Global Search</b><p className="text-sm text-slate-500">{results.length} hasil untuk “{query}”</p></div><Button variant="ghost" onClick={() => setOpen(false)}><X size={16} /></Button></div><div className="space-y-2">{results.map((r, i) => <div key={i} className="flex items-center justify-between rounded-xl border p-3"><div><Badge>{r.module}</Badge><p className="mt-1 font-semibold">{r.title}</p><p className="text-sm text-slate-500">{r.subtitle}</p><p className="text-xs text-slate-400">{r.date} • {money(r.amount)}</p></div><Button onClick={() => { onNavigate(r.page); setOpen(false); }}>Buka</Button></div>)}{results.length === 0 && <p className="text-sm text-slate-500">Ketik minimal 2 karakter untuk mencari dokumen, pasien, dokter, vendor, invoice, voucher, atau employee.</p>}</div></Dialog></header>;
}
