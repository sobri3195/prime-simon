...
export function AuditTrailPage() {
  ...
  const clear = () => { if (confirm('Hapus seluruh audit trail?')) { clearAuditTrail(); setRows([]); } };
  return <div><PageHeader title="Audit Trail" description="Log lokal create, update, delete, status-change, import, export, dan reset." actions={<><Button variant="outline" onClick={() => exportToCSV('audit-trail.csv', filtered as unknown as Record<string, unknown>[])}>Export CSV</Button><Button variant="destructive" onClick={clear}>Clear</Button></>} />
    <Alert className="mb-4">Audit trail tersimpan di localStorage dan menjaga 2.000 aktivitas terbaru.</Alert>
