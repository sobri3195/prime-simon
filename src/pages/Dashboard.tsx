...
export function Dashboard({ revenue, doctors, fees, ar, ap, payroll, settings }: { revenue: RevenueTransaction[]; doctors: Doctor[]; fees: DoctorFee[]; ar: ARItem[]; ap: APItem[]; payroll: PayrollRecord[]; settings: Settings }) {
  ...
  const exportAlertRows = async () => {
    ...
    const exportData = activeAlert === 'receivables' ? dueReceivables : activeAlert === 'payables' ? duePayables : unreconciledTransactions;
    ...
    link.href = url;
    link.download = activeAlert === 'receivables' ? 'piutang-jatuh-tempo-klinik-utama-prime-mata-2026-05.xlsx' : `${typeName}-klinik-utama-prime-mata-${fileDateSuffix}.xlsx`;
    link.click();
    URL.revokeObjectURL(url);
    toast.success(activeAlert === 'receivables' ? 'Export piutang jatuh tempo berhasil.' : 'Export berhasil dibuat.');
    } finally {
