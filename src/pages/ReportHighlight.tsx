...
import { calculateAchievement, generateReportHighlightAnalysis, reportHighlight } from '@/lib/calculations';
...
export function ReportHighlight({
  ...
  const breakdowns = breakdownInputs.map((item) => {
    ...
    const achievementPercent = achievement.achievementPercent;
    return {
      ...item,
      ...achievement,
      achievementPercent,
      status: achievementPercent >= 100 ? 'Tercapai' : 'Tidak Tercapai',
  ...
  const status = summary.achievementPercentage >= 100 ? 'Tercapai' : 'Tidak Tercapai';
  const analysis = generateReportHighlightAnalysis(summary, breakdowns, { monthName: monthNameID(month), year });
  ...
  return <div className="space-y-4 print-area"><PageHeader title="Report Highlight" description="Target vs realisasi dan analisis otomatis." actions={<div className="no-print flex items-center gap-2"><Select value={String(month)} onChange={(e) => setMonth(Number(e.target.value))}>{Array.from({ length: 12 }, (_, i) => <option key={i + 1} value={i + 1}>{monthNameID(i + 1)}</option>)}</Select><Select value={String(year)} onChange={(e) => setYear(Number(e.target.value))}>{[2025, 2026, 2027].map((y) => <option key={y} value={y}>{y}</option>)}</Select></div>} />
    {!hasPeriodData && !useMay2026Dummy ? <Alert>Belum ada analisis otomatis untuk periode ini karena data capaian belum tersedia.</Alert> : <>
      <Card className="print-card"><CardContent className="space-y-3"><p className="text-xs font-semibold uppercase tracking-wider text-blue-600">Klinik Utama Prime Mata · Finance Operations</p><p className="text-sm text-slate-500">Periode: {monthNameID(month)} {year}</p><div className="grid gap-4 md:grid-cols-4"><div><p>Total Target</p><b>{formatRupiah(summary.target)}</b></div><div><p>Total Realisasi</p><b>{formatRupiah(summary.realization)}</b></div><div><p>Capaian Total</p><b>{formatPercent(summary.achievementPercentage)}</b></div><div><p>Status Global</p><Badge className={getStatusBadgeClass(status)}>{status}</Badge></div></div><div className="h-2 rounded bg-slate-100"><div className={`h-2 rounded ${status === 'Tercapai' ? 'bg-emerald-500' : 'bg-red-500'}`} style={{ width: `${Math.min(summary.achievementPercentage, 100)}%` }} /></div></CardContent></Card>
