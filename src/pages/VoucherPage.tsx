...
export function VoucherPage({ rows, setRows }: { rows: Voucher[]; setRows: (r: Voucher[]) => void }) {
  ...
  return <section className="space-y-5">
    <header className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="max-w-3xl"><p className="mb-2 text-xs font-bold uppercase tracking-[0.18em] text-blue-600">{APP_NAME} • {MODULE_NAME}</p><h1 className="text-2xl font-extrabold tracking-tight text-slate-950 md:text-3xl">{PAGE_TITLE}</h1><p className="mt-2 text-sm leading-6 text-slate-500">CRUD lengkap dengan search/filter/sort, pagination, visibilitas kolom, export, print, dan penyimpanan data.</p></div><div className="flex flex-wrap items-center gap-2 lg:justify-end"><Button className="h-10 px-4" onClick={openCreate}><Plus size={16} />Tambah Voucher</Button><Button variant="outline" className="h-10 bg-white px-4" onClick={() => runExport('print')}><Printer size={16} /> Print</Button><Button variant="outline" className="h-10 bg-white px-4" onClick={resetDemo}><RotateCcw size={16} /> Reset Demo</Button></div></div>
    </header>
    ...
    <Card className="overflow-visible shadow-sm"><CardHeader><CardTitle>Daftar Voucher</CardTitle><p className="mt-1 text-sm text-slate-500">Data voucher siap digunakan dan tersimpan otomatis.</p></CardHeader><CardContent className="space-y-4">
