...
function normalizeVoucher(row: Voucher | Partial<NormalizedVoucher>): NormalizedVoucher {
  ...
  const voucherType = isVoucherType((row as any).voucherType) ? (row as any).voucherType : isVoucherType((row as any).type) ? (row as any).type : 'BBK';
  const amount = parseAmount(row.amount);
  const nomor = String((row as any).nomor || row.voucherNo || generateVoucherNumber(voucherType, tanggal, []));
  const terbilang = String((row as any).terbilang || row.amountInWords || rupiahTerbilang(amount));
  const nomorType = extractTypeFromNomor(nomor);
  if (nomorType && nomorType !== voucherType) console.warn(`Voucher type mismatch: voucherType ${voucherType} tetapi nomor voucher berisi ${nomorType}`);
  return {
...
function extractTypeFromNomor(nomor: string): VoucherType | null { const match = String(nomor || '').toUpperCase().match(/\/(BBK|BKK|KKM|KK|BKM|SB)\//); return match && isVoucherType(match[1]) ? match[1] : null; }
...
function getVoucherDate(row: Partial<NormalizedVoucher>) { return String((row as any).tanggal || (row as any).date || (row as any).transactionDate || (row as any).voucherDate || ''); }
function filterVouchersByDateRange(rows: NormalizedVoucher[], startDate: string, endDate: string) {
...
function filterVouchersByType(rows: NormalizedVoucher[], type: 'Semua' | VoucherType) { if (!type || type === 'Semua') return rows; return rows.filter((row) => row.type === type); }
function calculateVoucherSummary(rows: NormalizedVoucher[]) { return { totalVoucher: rows.length, approvedCount: rows.filter((row) => row.status === 'Approved').length }; }
function calculateVoucherTotalAmount(rows: NormalizedVoucher[]) { return rows.reduce((sum, row) => sum + row.amount, 0); }
function formatDateID(value: string) { return formatDate(value); }
...
export function VoucherPage({ rows, setRows }: { rows: Voucher[]; setRows: (r: Voucher[]) => void }) {
  ...
  const [startDate, setStartDate] = React.useState(() => loadFromStorage(START_DATE_KEY, '2026-05-01'));
  const [endDate, setEndDate] = React.useState(() => loadFromStorage(END_DATE_KEY, '2026-05-31'));
  ...
  const filteredRows = React.useMemo(() => {
    ...
    return byType.filter((row) => !query || voucherSearchText(row).includes(query));
  ...
  const totalAmount = React.useMemo(() => calculateVoucherTotalAmount(sortedRows), [sortedRows]);
  const summary = React.useMemo(() => calculateVoucherSummary(sortedRows), [sortedRows]);
  ...
  const hasActiveFilter = searchQuery.trim() !== '' || selectedType !== 'Semua' || startDate !== '2026-05-01' || endDate !== '2026-05-31';

  React.useEffect(() => { persistVouchers(vouchers, setRows); }, []); // migrate exact key once on mount
  React.useEffect(() => setCurrentPage(1), [searchQuery, selectedType, rowsPerPage, startDate, endDate]);
  ...
  React.useEffect(() => {
    if (startDate && endDate && startDate > endDate) setDateError('Tanggal awal tidak boleh lebih besar dari tanggal akhir.');
    else setDateError('');
  }, [startDate, endDate]);
  ...
  const getExportMeta = React.useCallback(() => ({ appName: APP_NAME, module: MODULE_NAME, page: PAGE_TITLE, exportedAt: new Date().toISOString(), period: activePeriod, totalRows: sortedRows.length, totalAmount, filters: { startDate, endDate, search: searchQuery, type: selectedType }, summary: { totalVoucher: summary.totalVoucher, totalAmount, approvedCount: summary.approvedCount }, sort: sortConfig, visibleColumns }), [activePeriod, startDate, endDate, searchQuery, selectedType, sortConfig, sortedRows.length, totalAmount, visibleColumns, summary]);
  ...
  const runExport = React.useCallback((type: 'csv' | 'excel' | 'json' | 'print') => {
    ...
    try {
      ...
      if (type === 'excel') exportToExcel({ rows: sortedRows, columns: exportColumns, filename: voucherFilename('xls'), sheetName: 'Voucher', includeFooter: true, meta });
  ...
  const resetFilter = () => { setSearchQuery(''); setSelectedType('Semua'); setStartDate('2026-05-01'); setEndDate('2026-05-31'); setSortConfig(defaultSort); setCurrentPage(1); };
  const resetDemo = React.useCallback(() => { [VOUCHER_KEY, VISIBLE_COLUMNS_KEY, ROWS_PER_PAGE_KEY, SELECTED_TYPE_KEY, SEARCH_KEY, SORT_KEY, ACTIVE_PERIOD_KEY, START_DATE_KEY, END_DATE_KEY].forEach(removeFromStorage); const demo = getDemoVouchers(); setVouchers(demo); persistVouchers(demo, setRows); setSearchQuery(''); setSelectedType('Semua'); setSortConfig(defaultSort); setRowsPerPage(10); setVisibleColumns(defaultVisibleColumns); setCurrentPage(1); setActivePeriod(DEFAULT_PERIOD); setStartDate('2026-05-01'); setEndDate('2026-05-31'); setFeedbackMessage('Data demo berhasil direset.'); }, [setRows]);
  ...
  return <section className="space-y-5">
    <header className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="max-w-3xl"><p className="mb-2 text-xs font-bold uppercase tracking-[0.18em] text-blue-600">{APP_NAME} • {MODULE_NAME}</p><h1 className="text-2xl font-extrabold tracking-tight text-slate-950 md:text-3xl">{PAGE_TITLE}</h1><p className="mt-2 text-sm leading-6 text-slate-500">CRUD lengkap dengan search/filter/sort, pagination, visibilitas kolom, export, dan print.</p></div><div className="flex flex-wrap items-center gap-2 lg:justify-end"><Button className="h-10 px-4" onClick={openCreate}><Plus size={16} />Tambah Voucher</Button><Button variant="outline" className="h-10 bg-white px-4" onClick={() => runExport('print')}><Printer size={16} /> Print</Button><Button variant="outline" className="h-10 bg-white px-4" onClick={resetDemo}><RotateCcw size={16} /> Reset Demo</Button></div></div>
    </header>
    <div className="grid gap-4 md:grid-cols-3"><SummaryMetricCard label="Total Voucher" value={`${summary.totalVoucher} dokumen`} meta={hasActiveFilter ? 'Mengikuti filter aktif' : 'Semua tipe voucher aktif'} /><SummaryMetricCard label="Total Amount" value={formatCurrency(totalAmount)} meta="Akumulasi nilai hasil aktif" tone="slate" /><SummaryMetricCard label="Status Approved" value={`${summary.approvedCount} Approved`} meta="Mengikuti hasil aktif" tone="green" /></div>
