...
import { generateDocumentNumber, generateVendorPaymentRequestNumber, isVendorPaymentRequestNumberFor } from '@/lib/numbering';
...
export function PaymentRequestPage({ rows, setRows, vendors }: { rows: PaymentRequest[]; setRows: (r: PaymentRequest[]) => void; vendors: Vendor[] }) {
  ...
  const getPreviewNumber = React.useCallback((draft: PaymentRequest) => {
    const existingRecord = rows.find((r) => r.id === draft.id);
    const unchangedExistingNumber = existingRecord?.requestNo && existingRecord.requestCategory === draft.requestCategory && existingRecord.requestDate === draft.requestDate && isVendorPaymentRequestNumberFor({ requestNo: existingRecord.requestNo, type: draft.requestCategory, date: draft.requestDate });
    if (unchangedExistingNumber) return existingRecord.requestNo;
    return generateVendorPaymentRequestNumber({ type: draft.requestCategory, date: draft.requestDate, existingRows: rows.filter((r) => r.id !== draft.id) });
  }, [rows]);

  const numberPreview = React.useMemo(() => editing ? getPreviewNumber(editing) : '', [editing, getPreviewNumber]);
  ...
  const save = () => {
    ...
    const shouldKeepExistingNumber = existingRecord?.requestNo && existingRecord.requestCategory === editing.requestCategory && existingRecord.requestDate === editing.requestDate && isVendorPaymentRequestNumberFor({ requestNo: existingRecord.requestNo, type: editing.requestCategory, date: editing.requestDate });
    let finalNo = shouldKeepExistingNumber ? existingRecord.requestNo : generateVendorPaymentRequestNumber({ type: editing.requestCategory, date: editing.requestDate, existingRows: rows.filter((r) => r.id !== editing.id) });
  ...
  return <div>
    <PageHeader title="Pengajuan Pembayaran Vendor" description="Klinik Utama Prime Mata • Finance Operations • Sequence nomor dipisah berdasarkan kategori Medis/Umum per bulan/tahun." actions={<Button onClick={openCreate}><Plus size={16} />Tambah</Button>} />
    {message && <Alert className="mb-4">{message}</Alert>}
    <DataTable rows={rows} filename={`pengajuan-pembayaran-vendor-${new Date().toISOString().slice(0, 10)}`} description='{"appName":"Klinik Utama Prime Mata","module":"Finance Operations","page":"Pengajuan Pembayaran Vendor","numberingRule":"Sequence nomor dipisah berdasarkan kategori Medis/Umum per bulan/tahun"}' columns={[
      { key: 'date', header: 'Tanggal', cell: r => r.requestDate },
      { key: 'no', header: 'Nomor', cell: r => r.requestNo },
      { key: 'type', header: 'Kategori', cell: r => <Badge variant={r.requestCategory === 'Medis' ? 'green' : 'outline'}>{r.requestCategory}</Badge>, exportAccessor: r => r.requestCategory },
    ...
    <Dialog open={!!editing}>{editing && <div className="space-y-4"><div><h2 className="text-lg font-bold">Form Pengajuan Pembayaran Vendor</h2></div>{error && <Alert>{error}</Alert>}<div className="grid gap-3 md:grid-cols-2">
      ...
      <label><span>Kategori Pengajuan</span><Select value={editing.requestCategory} onChange={e => setEditing({ ...editing, requestCategory: e.target.value as PaymentRequest['requestCategory'] })}><option>Medis</option><option>Umum</option></Select></label>
